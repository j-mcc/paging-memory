/*
 * $Author: o1-mccune $
 * $Date: 2017/12/02 04:35:25 $
 * $Revision: 1.2 $
 * $Log: simulatedclock.c,v $
 * Revision 1.2  2017/12/02 04:35:25  o1-mccune
 * fixed findAverage
 *
 * Revision 1.1  2017/12/01 20:10:58  o1-mccune
 * Initial revision
 *
...
void findAverage(sim_clock_t *average, int num){
  if(num == 0) return;
  if(average->seconds % num == 0){
    average->seconds = average->seconds / num;
    average->nanoseconds = average->nanoseconds / num;
  }
  else{
    float remainder = average->seconds / num;
    average->seconds = floor(average->seconds / num);
    remainder /= BILLION;
    average->nanoseconds /= num;
    if(average->nanoseconds + remainder > BILLION){
      average->seconds++;
      average->nanoseconds += remainder - BILLION;
    }
...
sim_clock_t findDifference(sim_clock_t *time1, sim_clock_t *time2){
  ...
  //fprintf(stderr, "\t\t\t\t(%d - %d) : (%d - %d) = (%d:%d)\n", time1->seconds, time2->seconds, time1->nanoseconds, time2->nanoseconds, difference.seconds, difference.nanoseconds);
  return difference;
}
